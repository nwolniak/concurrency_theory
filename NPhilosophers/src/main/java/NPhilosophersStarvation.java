...
public class NPhilosophersStarvation {
    private final int n;
    ...
    public NPhilosophersStarvation(int n, int feedingCounter) {
        this.n = n;
        ...
        try {
            this.writer = new FileWriter(String.format("C:\\Users\\Norbert\\Desktop\\tw\\lab2\\%s%d.csv", "starvation", this.n));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }

    public void runStarvationSim() {
        for (int i = 0; i < this.n; i++) {
            int finalI = i;
